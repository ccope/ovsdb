...
func parseSystemInfo(result Result) (map[string]string, error) {
	...
	if _, exists := systemInfo["system-id"]; exists == false {
		return systemInfo, fmt.Errorf("no 'system-id' found", nil)
	}
...
//	}
//	systemInfo, err := parseSystemInfo(result)
//	if err != nil {
